...
@router.get("/")
def root():
    return {"message": "Welcome to my world!! Curious to see where this goes!!! Its good"}


@router.post("/", status_code=status.HTTP_201_CREATED, response_model=schemas.PostResponse)
# Extract data sent in the body and store it in a python dictionary
...
@router.get("/p", response_model=List[schemas.PostOut])
...
def delete_post(id: int, db: Session = Depends(get_db), current_user: int = Depends(oauth2.get_current_user)):
    ...
    if post.owner_id != current_user.id:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN,
                            detail=f"Cannot perform the requested action")
